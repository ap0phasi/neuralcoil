...
step_swarm<-function(swarm_size,L=length(avec),w=0.9,g_p=0.4,g_g=0.4){
  ...
  n_v=matrix(runif(swarm_size*L,1e-9,1e-9),nrow=swarm_size,ncol=L)
  r_p=matrix(runif(swarm_size*L,0,0.1),nrow=swarm_size,ncol=L)
  r_g=matrix(runif(swarm_size*L,0,0.1),nrow=swarm_size,ncol=L)
