...
pop_coil<-function(input){
  model(input, training = TRUE)
  rdim<-dim(CoilVals)[1]
  val_out=model(input, training = TRUE)
  cnn_outputs <- as.array(val_out)
  rots<-(cnn_outputs[1:3]+1)*100
  stmat<-(matrix(cnn_outputs[(4):(4+n.s*2-1)],nrow=2))*10
  startvals<-complex(n.s,stmat[1,],stmat[2,])
  randmat<-matrix(cnn_outputs[(4+n.s*2):length(cnn_outputs)],nrow=2)
  RandVec<-complex(rdim,randmat[1,],randmat[2,])
  coil_out<-(runcoil(RandVec,rots,startvals))
...
dfw$objective[[1]]=apply(dfw$objective[[1]],2,function(x) (x-dfw$predictors[[objective]][1:length(x),lookback])/dfw$predictors[[objective]][1:length(x),lookback])
dfw$objective[[1]][is.na(dfw$objective[[1]])]=0
dfw$objective[[1]][is.infinite(dfw$objective[[1]])]=0
dfw$objective[[1]][(dfw$objective[[1]])<(-0.05)]=-0.05
dfw$objective[[1]][(dfw$objective[[1]])>(0.05)]=0.05

scaledat<-scalelist(dfw,objrange=c(0.2,0.8))
...
assign_weights(weights,weightdim,bestp[which.min(outgs),])
